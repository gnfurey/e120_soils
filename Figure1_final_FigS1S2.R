...
#
AppendixS3col <- colnames(e120)
write.csv(x = AppendixS3col,file = "Data/S3_DatasetS1.columns.csv",row.names = FALSE)
#check mono
mono <- e120 %>% 
  filter(NumSp==1) %>% 
  select(Plot,Species) 
table(mono$Species)
###############
#colnames(e120)#check column names
...
mod <- lm(gm2_2017_0.20cm_Potassium_aa~log(NumSp),data=e120)
...
plot_point <- function(x){
  ...
  p1 <- ggplot(tmp,aes(x=log(NumSp),
    ...
    ylab("")+
    # scale_y_continuous(expand = expansion(0.04,0),
    #                    limits=c(0,NA))+
    scale_y_continuous(expand = expansion(0.04,0))+
...
p1 <- p1+
  scale_y_continuous(breaks=seq(from=1200,2200,by=50),
                     limits=c(1200,2200),
                     labels=every_nth(seq(from=1200,2200,by=50),4,inverse = TRUE))
p1
...
p2 <- p2+ scale_y_continuous(breaks=seq(from=120,180,by=5),
                        limits=c(115,180),
                        labels=every_nth(seq(from=120,180,by=5),4,
                                         inverse = TRUE))

p2
p3 <- plot_point("Potassium")
p3 <- p3+scale_y_continuous(breaks=seq(from=7,15,by=0.5),
                     limits=c(7,15),
                     labels=every_nth(seq(from=7,15,by=0.5),2,inverse = TRUE))

p3
p4 <- plot_point("Calcium")
p4 <- p4+scale_y_continuous(breaks=seq(from=100,165,by=5),
                     limits=c(95,165),
                     labels=every_nth(seq(from=100,165,by=5),4,inverse = TRUE))

p4
p5 <- plot_point("Magnesium")
p5
p5 <- p5+scale_y_continuous(breaks=seq(from=14,27,by=1),
                     limits=c(14,27),
                     labels=every_nth(seq(from=14,27,by=1),4,inverse = TRUE))
p5
p6 <- plot_point("CEC")
p6
p6 <- p6+scale_y_continuous(breaks=seq(from=2.2,3.9,by=0.1),
                      limits=c(2.2,3.9),
                      labels=every_nth(seq(from=2.2,3.9,by=0.1),4,inverse = TRUE))

p6
p7 <- plot_point("pH")
p7 <- p7+scale_y_continuous(breaks=seq(from=5.3,6.3,by=0.1),
                      limits=c(5.3,6.3),
...
p1 <- labelfun(plot0 = p1,
         ...
         left = 0,
         bottom = 0,
         ylab1 = expression(paste("Carbon ","(","g"%.%"m"^-2,")")))
# 
p2 <- labelfun(plot0 = p1,
            title1 = "b",
            top = 0,
            right = 0.8,
            left = 0,
            bottom = 0,
            ylab1 = expression(paste("Nitrogen ","(","g"%.%"m"^-2,")")))
# p3
p3 <- labelfun(plot0 = p3,
               ...
               left = 0,
               bottom = 0,
               ylab1 = expression(paste("Potassium ","(","g"%.%"m"^-2,")")))
p4 <- labelfun(plot0 = p4,
               ...
               left = 0,
               bottom = 0,
               ylab1 = expression(paste("Calcium ","(","g"%.%"m"^-2,")")))
p4
p5 <- labelfun(plot0 = p5,
               ...
               left = 0,
               bottom = 0,
               ylab1 = expression(paste("Magnesium ","(","g"%.%"m"^-2,")")))
...
phos1 <- labelfun(plot0 = phos,
         ...
         left = 0,
         bottom = 0,
         ylab1 = expression(paste("Phosphorus ","(","g"%.%"m"^-2,")")))
phos1 <- phos1+
  scale_y_continuous(breaks=seq(from=8,16,by=0.5),
                     limits=c(8,16),
                     labels=every_nth(seq(from=8,16,by=0.5),4,inverse = TRUE))
phos1
###### 
ft2 <- 12
#generate final plot
...
# #
ggsave(filename = "Figures_PRINT/Figure1.pdf",
       # device = cairo_ps,
       dpi = 600,
       plot = grid.arrange(
         mylegend.1,
...
check <- grid.arrange(
  ...
  heights=list(unit(7,"mm"),unit(93,"mm")),
  padding=unit(3,"mm"))
saveRDS(check,"Figures_PRINT/Rds/Figure1.rds")
###########
###########
library(flextable)
library(officer)
...
#
table$nut <- ifelse(table$nut=="pH.waypoint","pH",
                           table$nut)
...
print(doc, target = "Tables/SupplementalTable2_raw.docx")
#########
...
b1 <- b1+theme(legend.position = "none")+
  ...
  ylab(expression(paste("Carbon ","(","g"%.%"m"^-2,")")))
# 
...
b2 <- b2+theme(legend.position = "none")+
  ...
  theme(plot.title = element_text(face="bold",family="Helvetica",size=8,
                                  margin=margin(0,0,0,0)),
        plot.margin = margin(0,0.8,0,0))+
  ylab(expression(paste("Nitrogen ","(","g"%.%"m"^-2,")")))
#
...
b3 <- b3+theme(legend.position = "none")+
  ...
  theme(plot.title = element_text(face="bold",family="Helvetica",size=8,
                                  margin=margin(0,0,0,0)),
        plot.margin = margin(0,0.8,0,0))+
  ylab(expression(paste("Potassium ","(","g"%.%"m"^-2,")")))
#
...
b4 <- b4+theme(legend.position = "none")+
  ...
  theme(plot.title = element_text(face="bold",family="Helvetica",size=8,
                                  margin=margin(0,0,0,0)),
        plot.margin = margin(0,0.8,0,0))+
  ylab(expression(paste("Calcium ","(","g"%.%"m"^-2,")")))
#
...
b5 <- b5+theme(legend.position = "none")+
  ...
  theme(plot.title = element_text(face="bold",family="Helvetica",size=8,
                                  margin=margin(0,0,0,0)),
        plot.margin = margin(0,0.8,0,0))+
  ylab(expression(paste("Magnesium ","(","g"%.%"m"^-2,")")))
#
...
b8 <- b8+theme(legend.position = "none")+
  ...
  theme(plot.title = element_text(face="bold",family="Helvetica",size=8,
        ...
        plot.margin = margin(0,0.8,0,0))+
  ylab(expression(paste("Phosphorus ","(","g"%.%"m"^-2,")")))+
  scale_y_continuous(expand = expansion(0.1,0.1))
#
ft2 <- 8
#
